...
class MAF(tf.keras.models.Model):
    ...
    def __init__(self,num_masked=1,output_dim=2,num_layers=4,neuron_list=[50,50], ref_dist = None):
        super(MAF, self).__init__(name='MAF')
        ...
        for i in range(num_layers):
                self.shift_and_log_scale_fn[i] = tfb.AutoregressiveNetwork(params=output_dim, hidden_units=neuron_list)

        bijectors = []
        # for floweach layer containing bijector, batch_norm, and permutation
        for i in range(num_layers):

            bijectors.append(tfb.Invert(tfb.MaskedAutoregressiveFlow(shift_and_log_scale_fn=self.shift_and_log_scale_fn[i])))
            if (i < (num_layers - 1)):
